...
describe('Attributor', function() {
  it('build', function() {
    let blot = EditorRegistry.create('inline');
  ...
  it('add to text', function() {
    let container = EditorRegistry.create('block');
    let textBlot = EditorRegistry.create('text', 'Test');
  ...
  it('remove', function() {
    ...
    let boldBlot = EditorRegistry.create(node);
    container.appendChild(boldBlot);
  ...
  it('invalid class scope add', function() {
    let inline = EditorRegistry.create('inline');
    let blockAttributor = EditorRegistry.query('indent');
    blockAttributor.add(inline.domNode, 1, EditorRegistry);
  ...
  it('missing class value', function() {
    ...
    let indentAttributor = EditorRegistry.query('indent');
    expect(indentAttributor.value(block.domNode, EditorRegistry)).toBeFalsy();
  });

  it('removes quotes from attribute value when checking if canAdd', function () {
    let node = document.createElement('strong');
    let familyAttributor = EditorRegistry.query('family');
    expect(familyAttributor.canAdd(node, 'Arial', EditorRegistry)).toBeTruthy();
    expect(familyAttributor.canAdd(node, '"Times New Roman"', EditorRegistry)).toBeTruthy();
    expect(familyAttributor.canAdd(node, 'monotype', EditorRegistry)).toBeFalsy();
    expect(familyAttributor.canAdd(node, '"Lucida Grande"', EditorRegistry)).toBeFalsy();
  });
});
